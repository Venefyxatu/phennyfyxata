...
def activewars(phenny, input): 
    """ Ik zal een overzicht geven van de wars die nog moeten komen """
    # Print statement to work around a bug when input.group(2) doesn't exist
    print "%s New command received in activewars module" % time.localtime()
    arguments = input.group(2)
    command = input.group(1)
    ...
    djangoUrl = "http://phenny.venefyxatu.be/wars/active/"
    url = openUrl(djangoUrl)

    active_wars = url.read()
    if active_wars:
        phenny.say("Deze wars zijn bezig:")
    else:
        phenny.say("Er zijn geen wars bezig")
        return
    for war in active_wars.split(','):
        phenny.say(war)
    return

activewars.commands = ["activewars"]
activewars.example = '.activewars'
